...
if (strcmp(dev.CPUArchitecture, "arm64") &&
    strcmp(dev.CPUArchitecture, "arm64e")) {
    LOG(LOG_WARNING,
...
static void* connected_dfu_mode(struct irecv_device_info* info) {
	...
	/* A12/A13 and S4/S5 cannot use checkm8.  When --pwned-dfu is
	 * supplied, route the three tested usbliter8 CPIDs directly to the
	 * libusb transport instead of falling through to checkra1n. */
	if (palerain_flags & palerain_option_pwned_dfu) {
		...
		if (irecv_open_with_ecid(&device, info->ecid) == IRECV_E_SUCCESS) {
			pwned = detect_pwned_dfu(device);
			if (pwned == 1) {
				LOG(LOG_INFO, "Pwned DFU detected");
				if (cpid_needs_usbliter8(cpid)) {
					const char *chip = usbliter8_cpid_name(cpid);
					if (detect_usbliter8_dfu(device) != 1) {
						LOG(LOG_ERROR,
							"CPID 0x%04x (%s) is not marked as usbliter8 pwned",
							cpid, chip ? chip : "usbliter8");
					} else {
						LOG(LOG_INFO, "usbliter8 %s device found; sending PongoOS",
							chip ? chip : "supported");
						/* irecv and libusb must not hold the same DFU interface. */
						irecv_close(device);
						device = NULL;
						if (boot_usbliter8() == 0) {
							usbliter8_booted = true;
							device_has_booted = true;
							/* Leave the Pongo listener running while DFU re-enumerates. */
							unsubscribe_cmd();
							pthread_exit(NULL);
							return NULL;
						}
						LOG(LOG_ERROR, "usbliter8 boot request failed");
					}
				}
			} else if (pwned == 0) {
				LOG(LOG_ERROR, "Device is not in pwned DFU mode");
				/* Keep --pwned-dfu set so main() cannot run checkra1n on an
				 * A12/A13-class device after this failed precondition. */
			} else {
				LOG(LOG_ERROR, "Could not read the DFU serial string");
			}
			if (device)
				irecv_close(device);
		} else {
			LOG(LOG_ERROR, "Could not open DFU device");
		}
	}
...
void *dfuhelper(void* ptr) {
	...
	set_spin(1);
	ret = subscribe_cmd(device_event_cb, irecv_device_event_cb);
	if (ret != 0) {
		LOG(LOG_ERROR, "Unable to subscribe to device events: %d", ret);
		set_spin(0);
		dfuhelper_thr_running = false;
		return NULL;
	}
	while (get_spin() && !usbliter8_booted) {
		sleep(1);
	};
	dfuhelper_thr_running = false;
	return 0;
