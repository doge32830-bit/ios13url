...
bool cpid_is_arm64(unsigned int cpid) {
	...
	*        A12 (usbliter8)
	* ========== 0x8027 ===========
	*        A12X/Z (not implemented by usbliter8 yet)
	* ========== 0x8030 ===========
	*        A13 (usbliter8)
	* ========== 0x8700 ===========
	*          iPod SoCs
	...
	*/
	return (
		...
		cpid == 0x8006 || cpid == 0x8010 || cpid == 0x8011 ||
		cpid == 0x8012 || cpid == 0x8015 || cpid == 0x8020 ||
		cpid == 0x8027 || cpid == 0x8030
	);
}

bool cpid_needs_usbliter8(unsigned int cpid) {
	return is_usbliter8_cpid(cpid);
}
