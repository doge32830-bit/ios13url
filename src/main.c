...
static int palera1n(int argc, char *argv[], char *envp[]) {
	bool pongo_thread_started = false;
	...
	if (simulator_is_enabled()) {
		LOG(LOG_WARNING, "A12/A13 devices require usbliter8 first, then run palera1n again while the device is in pwned DFU.");
		goto cleanup;
	}

	ret = pthread_create(&pongo_thread, NULL, pongo_helper, NULL);
	if (ret != 0) {
		LOG(LOG_ERROR, "Cannot start PongoOS listener: %s", strerror(ret));
		goto cleanup;
	}
	pongo_thread_started = true;

	ret = pthread_create(&dfuhelper_thread, NULL, dfuhelper, NULL);
	if (ret != 0) {
		LOG(LOG_ERROR, "Cannot start DFU listener: %s", strerror(ret));
		goto cleanup;
	}
	ret = pthread_join(dfuhelper_thread, NULL);
	if (ret != 0) {
		LOG(LOG_ERROR, "Cannot join DFU listener: %s", strerror(ret));
		goto cleanup;
	}
	/* A usbliter8 boot disconnects DFU and then presents PongoOS.  Keep the
	 * Pongo listener alive until it has consumed that device; otherwise the
	 * original port exits immediately after the USBLiter8 request. */
	if (usbliter8_booted) {
		ret = pthread_join(pongo_thread, NULL);
		pongo_thread_started = false;
		if (ret != 0) {
			LOG(LOG_ERROR, "Cannot join PongoOS listener: %s", strerror(ret));
			goto cleanup;
		}
		goto normal_exit;
	}
	...
	if ((palerain_flags & (palerain_option_dfuhelper_only |
							  ...
							  palerain_option_pwned_dfu)
						 ) || device_has_booted)
		goto normal_exit;
	if (pongo_thread_started) {
	...
	if (simulator_is_enabled())
		ret = pthread_create(&pongo_thread, NULL, simulator_pongo_helper, NULL);
	else
		ret = pthread_create(&pongo_thread, NULL, pongo_helper, NULL);
	if (ret != 0) {
		LOG(LOG_ERROR, "Cannot start PongoOS worker: %s", strerror(ret));
		goto cleanup;
	}
	pongo_thread_started = true;
	ret = pthread_join(pongo_thread, NULL);
	pongo_thread_started = false;
	if (ret != 0) {
		LOG(LOG_ERROR, "Cannot join PongoOS worker: %s", strerror(ret));
		goto cleanup;
	}
	while (get_spin())
	{
...
cleanup:
	...
	if (pongo_path != NULL) {
		/* --override-pongo is owned by the option parser. */
		free(pongo_path);
		pongo_path = NULL;
	}
